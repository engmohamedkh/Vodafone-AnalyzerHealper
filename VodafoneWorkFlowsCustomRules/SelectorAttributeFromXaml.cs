...
namespace SelectorAttributeFromXamlSpace
{
    ...
    {
        const string RuleID = "VF-063";
        const string RuleName = "SelectorAttributeFromXaml";
                ...
                foreach (var (selectorValue, position) in EnumerateSelectorsInXaml(xamlContent))
                {
                    ...
                    if (unstable.Count > 0)
                    {
                        ...
                        string activityLabel = !string.IsNullOrWhiteSpace(activityName)
                            ...
                            : "Selector";
                        messageList.Add(string.Format("{0} uses unstable attribute(s): {1}. Use stable attributes: {2}.",
                            activityLabel, string.Join(", ", unstable), RecommendedAttributes));
                    }
                }
            }
            catch (Exception)
