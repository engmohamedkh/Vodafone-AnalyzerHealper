...
namespace SelectorAttributeValueFromXamlSpace
{
	...
	{
		const string RuleID = "VF-057";
		const string RuleName = "static Selector";
		...
		public void Initialize(IAnalyzerConfigurationService workflowAnalyzerConfigService)
		{
			...
			var newRule = new Rule<IWorkflowModel>(RuleName, RuleID, Evaluate)
			{
				...
				RecommendationMessage = "Selector attribute values for " + string.Join(", ", AttributesValueMustContainWildcard) + " must contain a wildcard (*, ?, or #) and cannot be only wildcard characters."
			};
		...
		/// <summary>Finds attributes with missing wildcard or values made only of wildcard characters.</summary>
		private static SelectorAttributeIssue FindSelectorAttributeIssues(string selectorXml)
		{
			var issue = new SelectorAttributeIssue
			{
				MissingWildcardText = null,
				WildcardOnlyText = null
			};
			...
			foreach (Match m in AttributeValueRegex.Matches(decoded))
			{
				if (m.Success && m.Groups.Count >= 3)
				{
					...
					{
						missingWildcard.Add(string.Format("{0}=\"{1}\"", attrName, value));
					}
					else if (value.All(c => WildcardChars.Contains(c)))
					{
						wildcardOnly.Add(string.Format("{0}=\"{1}\"", attrName, value));
					}
				}
			}
		...
		private InspectionResult Evaluate(IWorkflowModel workflow, Rule theNewRule)
		{
			...
			try
			{
				...
				{
					SelectorAttributeIssue issue = FindSelectorAttributeIssues(selectorValue);
					if (!string.IsNullOrEmpty(issue.MissingWildcardText) || !string.IsNullOrEmpty(issue.WildcardOnlyText))
					{
						...
						{
							messageList.Add(string.Format("{0} has selector attribute value(s) without a wildcard (*, ?, #): {1}.",
								activityLabel, issue.MissingWildcardText));
						}
						if (!string.IsNullOrEmpty(issue.WildcardOnlyText))
						{
							messageList.Add(string.Format("{0} has selector attribute value(s) made only of wildcard characters (*, ?, #): {1}.",
								activityLabel, issue.WildcardOnlyText));
						}
					}
				}
			}
